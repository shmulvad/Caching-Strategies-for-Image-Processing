...
class MortonOrder(CachingDataStructure):
    ...
    def __set_part_compact_func__(self) -> None:
        ...
        """
        if self.dim <= 4:  # Hardcoded case which is a little faster
            part_compact_to_use = [
        ...
        else:  # Dynamic case which works for every n >= 1 but is a bit slower
            n = self.dim - 1
            self.part_func, self.compact_func = self.gen_part_compact_funcs(n)
    ...
    def __get_hex_const__(self, successive_digits: int, spacing: int,
        ...
        >>> get_hex_const(2, 3, 16)
        0b1000110001100011
        >>> get_hex_const(1, 4, 16)
        0b1000010000100001
        """
        hex_num, bit_idx = 0, 0
    ...
    def gen_part_compact_funcs(self, n: int, num_bits: int = 32) \
            -> Tuple[Callable[[int], int], Callable[[int], int]]:
        """
        Returns a tuple of part1byN and compact1byN functions for a given n
        and number of bits
        """
    ...
    def compact1by3(self, x: int) -> int:
        ...
        """
        # Modified based on http://asgerhoedt.dk/?p=276
